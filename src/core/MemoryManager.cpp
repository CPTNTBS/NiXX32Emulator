 ...
 */
 ...
 uint8_t MemoryManager::Read8(uint32_t address) {
	// Simulate access delay for address decoding
	SimulateAccessDelay(address, false, 8);
 ...
 MemoryRegion* MemoryManager::DefineRegion(const std::string& name, uint32_t startAddress, 
	...
	// Address Decoding Simulation Initialization:
	region.isMirrored = false;
	region.mirrorMask = 0;
	region.bankable = false;
	region.currentBankIndex = 0;
	region.accessLatency = 1;  // Default 1 cycle latency
	region.useVariableLatency = false;	
...
bool MemoryManager::ReconfigureRegion(const std::string& name, uint32_t newSize, MemoryAccess newAccess) {
	std::unique_lock<std::shared_mutex> lock(m_memoryMutex);
	
	auto it = m_regionsByName.find(name);
	if (it == m_regionsByName.end()) {
		m_logger.Error("MemoryManager", "Cannot reconfigure region: " + name + " - not found");
		return false;
	}
	
	MemoryRegion& region = m_regions[it->second];
	
	// Save original data to restore as much as possible
	std::vector<uint8_t> originalData = region.data;
	
	// Resize the region
	region.size = newSize;
	region.data.resize(newSize, 0);
	
	// Copy back original data, up to the smaller of old size or new size
	size_t copySize = std::min(originalData.size(), region.data.size());
	std::copy(originalData.begin(), originalData.begin() + copySize, region.data.begin());
	
	// Update access permissions
	region.access = newAccess;
	
	// Update refresh parameters if needed
	if (region.needsRefresh) {
		region.refreshRowCount = (region.size + region.refreshRowSize - 1) / region.refreshRowSize;
		region.refreshMap.resize(region.refreshRowCount, false);
	...
	m_logger.Info("MemoryManager", "Region reconfigured: " + name + 
				 ", new size: " + std::to_string(newSize) + 
				 ", new access: " + std::to_string(static_cast<int>(newAccess)));
	
	return true;
}

bool MemoryManager::ResizeRegion(const std::string& name, uint32_t newSize) {
	std::unique_lock<std::shared_mutex> lock(m_memoryMutex);
	
	auto it = m_regionsByName.find(name);
	if (it == m_regionsByName.end()) {
		m_logger.Error("MemoryManager", "Cannot resize region: " + name + " - not found");
		return false;
	}
	
	MemoryRegion& region = m_regions[it->second];
	MemoryAccess currentAccess = region.access;
	
	return ReconfigureRegion(name, newSize, currentAccess);
}

bool MemoryManager::ChangeRegionAccess(const std::string& name, MemoryAccess newAccess) {
	std::unique_lock<std::shared_mutex> lock(m_memoryMutex);
	
	auto it = m_regionsByName.find(name);
	if (it == m_regionsByName.end()) {
		m_logger.Error("MemoryManager", "Cannot change region access: " + name + " - not found");
		return false;
	}
	...
	m_logger.Info("MemoryManager", "Region access changed: " + name + 
				 ", new access: " + std::to_string(static_cast<int>(newAccess)));
	
	return true;
}

bool MemoryManager::RelocateRegion(const std::string& name, uint32_t newStartAddress) {
	std::unique_lock<std::shared_mutex> lock(m_memoryMutex);
	
	auto it = m_regionsByName.find(name);
	if (it == m_regionsByName.end()) {
		m_logger.Error("MemoryManager", "Cannot relocate region: " + name + " - not found");
		return false;
	}
	
	// Make sure the new location doesn't overlap with existing regions
	for (const auto& region : m_regions) {
		uint32_t regionEnd = region.startAddress + region.size - 1;
		uint32_t newEnd = newStartAddress + m_regions[it->second].size - 1;
		
		// Check if regions would overlap
		bool overlaps = !(newEnd < region.startAddress || newStartAddress > regionEnd);
		
		if (overlaps && region.name != name) {
			m_logger.Error("MemoryManager", "Cannot relocate region: " + name + 
						 " - would overlap with region: " + region.name);
			return false;
		}
	}
	
	// Update the region's start address
	m_regions[it->second].startAddress = newStartAddress;
	
	m_logger.Info("MemoryManager", "Region relocated: " + name + 
				 ", new start address: 0x" + std::to_string(newStartAddress));
	
	return true;
}

bool MemoryManager::CreateMirror(const std::string& sourceName, uint32_t mirrorAddress) {
	std::unique_lock<std::shared_mutex> lock(m_memoryMutex);
	
	auto it = m_regionsByName.find(sourceName);
	if (it == m_regionsByName.end()) {
		m_logger.Error("MemoryManager", "Cannot create mirror: source region " + 
					   sourceName + " not found");
		return false;
	}
	
	MemoryRegion& sourceRegion = m_regions[it->second];
	
	// Check for overlapping regions at mirror address
	for (const auto& region : m_regions) {
		uint32_t regionEnd = region.startAddress + region.size - 1;
		uint32_t mirrorEnd = mirrorAddress + sourceRegion.size - 1;
		
		bool overlaps = !(mirrorEnd < region.startAddress || mirrorAddress > regionEnd);
		
		if (overlaps) {
			m_logger.Error("MemoryManager", "Cannot create mirror: would overlap with region " + 
						   region.name);
			return false;
		}
	}
	
	// Mark the region as mirrored
	sourceRegion.isMirrored = true;
	sourceRegion.mirrorAddresses.push_back(mirrorAddress);
	
	// If no mirror mask is set, use a default that covers the entire region
	if (sourceRegion.mirrorMask == 0) {
		sourceRegion.mirrorMask = sourceRegion.size - 1;
	}
	
	m_logger.Info("MemoryManager", "Mirror created for region " + sourceName + 
				  " at address 0x" + std::to_string(mirrorAddress));
	
	return true;
}

bool MemoryManager::RemoveMirror(const std::string& sourceName, uint32_t mirrorAddress) {
	std::unique_lock<std::shared_mutex> lock(m_memoryMutex);
	
	auto it = m_regionsByName.find(sourceName);
	if (it == m_regionsByName.end()) {
		m_logger.Error("MemoryManager", "Cannot remove mirror: source region " + 
					   sourceName + " not found");
		return false;
	}
	...
	// Find and remove the mirror address
	auto mirrorIt = std::find(sourceRegion.mirrorAddresses.begin(), 
							  sourceRegion.mirrorAddresses.end(), mirrorAddress);
	
	if (mirrorIt == sourceRegion.mirrorAddresses.end()) {
		m_logger.Error("MemoryManager", "Cannot remove mirror: mirror address 0x" + 
					   std::to_string(mirrorAddress) + " not found");
		return false;
	}
	
	sourceRegion.mirrorAddresses.erase(mirrorIt);
	
	// If no mirrors left, mark as not mirrored
	if (sourceRegion.mirrorAddresses.empty()) {
		sourceRegion.isMirrored = false;
	}
	
	m_logger.Info("MemoryManager", "Mirror removed for region " + sourceName + 
				  " at address 0x" + std::to_string(mirrorAddress));
	
	return true;
}

void MemoryManager::SetMirrorMask(const std::string& name, uint32_t mask) {
	std::unique_lock<std::shared_mutex> lock(m_memoryMutex);
	
	auto it = m_regionsByName.find(name);
	if (it == m_regionsByName.end()) {
		m_logger.Error("MemoryManager", "Cannot set mirror mask: region " + name + " not found");
		return;
	}
	
	m_regions[it->second].mirrorMask = mask;
	
	m_logger.Info("MemoryManager", "Mirror mask set for region " + name + 
				  " to 0x" + std::to_string(mask));
}

int MemoryManager::FindRegionIndex(uint32_t address) {
	// First check direct regions
	...
	// Then check mirrored regions
	for (size_t i = 0; i < m_regions.size(); i++) {
		const auto& region = m_regions[i];
		if (region.isMirrored) {
			for (uint32_t mirrorAddress : region.mirrorAddresses) {
				if (address >= mirrorAddress && address < mirrorAddress + region.size) {
					return static_cast<int>(i);
				}
			}
		}
...
uint32_t MemoryManager::GetRegionRelativeAddress(uint32_t address, int regionIndex) {
	...
	// If the address is within the primary region
	if (address >= region.startAddress && address < region.startAddress + region.size) {
		return address - region.startAddress;
	}
	
	// If mirrored, check mirror addresses
	if (region.isMirrored) {
		for (uint32_t mirrorAddress : region.mirrorAddresses) {
			if (address >= mirrorAddress && address < mirrorAddress + region.size) {
				// Apply mirror mask to handle special mirroring cases
				return (address - mirrorAddress) & region.mirrorMask;
			}
		}
	}
...
bool MemoryManager::AddMemoryBank(const std::string& regionName, uint32_t bankId, uint32_t size) {
	std::unique_lock<std::shared_mutex> lock(m_memoryMutex);
	
	auto it = m_regionsByName.find(regionName);
	if (it == m_regionsByName.end()) {
		m_logger.Error("MemoryManager", "Cannot add memory bank: region " + regionName + " not found");
		return false;
	}
	...
	for (const auto& bank : region.banks) {
		if (bank.bankId == bankId) {
			m_logger.Error("MemoryManager", "Cannot add memory bank: bank ID " + 
						   std::to_string(bankId) + " already exists");
			return false;
		}
	}
	...
	// Create new bank
	MemoryBank newBank;
	newBank.name = regionName + "_bank_" + std::to_string(bankId);
	newBank.bankId = bankId;
	newBank.size = size;
	newBank.data.resize(size, 0);
	
	region.banks.push_back(newBank);
	region.bankable = true;
	
	// If this is the first bank, make it active
	if (region.banks.size() == 1) {
		region.currentBankIndex = 0;
		
		// Copy the main region data to the bank as initial content
		std::copy(region.data.begin(), 
				 region.data.begin() + std::min(region.size, size), 
				 newBank.data.begin());
	}
	
	m_logger.Info("MemoryManager", "Memory bank added: " + newBank.name + 
				  ", size: " + std::to_string(size));
	
	return true;
}

bool MemoryManager::SwitchBank(const std::string& regionName, uint32_t bankId) {
	std::unique_lock<std::shared_mutex> lock(m_memoryMutex);
	
	auto it = m_regionsByName.find(regionName);
	if (it == m_regionsByName.end()) {
		m_logger.Error("MemoryManager", "Cannot switch bank: region " + regionName + " not found");
		return false;
	}
	
	MemoryRegion& region = m_regions[it->second];
	
	if (!region.bankable || region.banks.empty()) {
		m_logger.Error("MemoryManager", "Cannot switch bank: region " + regionName + 
					   " is not bankable or has no banks");
		return false;
	}
	
	// Find the bank with the given ID
	for (size_t i = 0; i < region.banks.size(); i++) {
		if (region.banks[i].bankId == bankId) {
			// Save current bank data
			std::copy(region.data.begin(), 
					 region.data.begin() + std::min(region.size, region.banks[region.currentBankIndex].size), 
					 region.banks[region.currentBankIndex].data.begin());
			
			// Switch bank
			region.currentBankIndex = i;
			
			// Copy new bank data to main region
			std::copy(region.banks[i].data.begin(), 
					 region.banks[i].data.begin() + std::min(region.size, region.banks[i].size), 
					 region.data.begin());
			
			m_logger.Info("MemoryManager", "Switched to bank ID " + std::to_string(bankId) + 
	...
	m_logger.Error("MemoryManager", "Cannot switch bank: bank ID " + std::to_string(bankId) + 
				   " not found");
	return false;
}

bool MemoryManager::LoadBankData(const std::string& regionName, uint32_t bankId, 
								const std::vector<uint8_t>& data) {
	std::unique_lock<std::shared_mutex> lock(m_memoryMutex);
	
	auto it = m_regionsByName.find(regionName);
	if (it == m_regionsByName.end()) {
		m_logger.Error("MemoryManager", "Cannot load bank data: region " + 
	...
	for (auto& bank : region.banks) {
		if (bank.bankId == bankId) {
			// Check if data fits in the bank
			if (data.size() > bank.size) {
				m_logger.Warning("MemoryManager", "Bank data size exceeds bank size, truncating");
			}
			
			// Copy data to bank
			size_t copySize = std::min(data.size(), static_cast<size_t>(bank.size));
			std::copy(data.begin(), data.begin() + copySize, bank.data.begin());
			
			// If this is the active bank, update the main region data too
			if (&bank == &region.banks[region.currentBankIndex]) {
				size_t regionCopySize = std::min(copySize, static_cast<size_t>(region.size));
				std::copy(data.begin(), data.begin() + regionCopySize, region.data.begin());
			}
			
			m_logger.Info("MemoryManager", "Bank data loaded for bank ID " + 
						  std::to_string(bankId) + " in region " + regionName);
			
			return true;
		}
	}
	
	m_logger.Error("MemoryManager", "Cannot load bank data: bank ID " + 
				   std::to_string(bankId) + " not found");
	return false;
}

uint32_t MemoryManager::GetCurrentBank(const std::string& regionName) {
	std::shared_lock<std::shared_mutex> lock(m_memoryMutex);
	
	auto it = m_regionsByName.find(regionName);
	if (it == m_regionsByName.end() || 
		!m_regions[it->second].bankable || 
		m_regions[it->second].banks.empty()) {
		return UINT32_MAX;  // Invalid bank ID
	}
	
	const MemoryRegion& region = m_regions[it->second];
	return region.banks[region.currentBankIndex].bankId;
}

void MemoryManager::SetRegionLatency(const std::string& name, uint32_t latencyCycles) {
	std::unique_lock<std::shared_mutex> lock(m_memoryMutex);
	
	auto it = m_regionsByName.find(name);
	if (it == m_regionsByName.end()) {
		m_logger.Error("MemoryManager", "Cannot set latency: region " + name + " not found");
		return;
	}
	
	m_regions[it->second].accessLatency = latencyCycles;
	m_regions[it->second].useVariableLatency = false;
	
	m_logger.Info("MemoryManager", "Latency set to " + std::to_string(latencyCycles) + 
				  " cycles for region " + name);
}

void MemoryManager::SetVariableLatency(const std::string& name, 
									 std::function<uint32_t(uint32_t)> calculator) {
	std::unique_lock<std::shared_mutex> lock(m_memoryMutex);
	
	auto it = m_regionsByName.find(name);
	if (it == m_regionsByName.end()) {
		m_logger.Error("MemoryManager", "Cannot set variable latency: region " + 
					   name + " not found");
		return;
	}
	
	m_regions[it->second].latencyCalculator = calculator;
	m_regions[it->second].useVariableLatency = true;
	
	m_logger.Info("MemoryManager", "Variable latency set for region " + name);
}

uint32_t MemoryManager::GetAccessLatency(uint32_t address) {
	std::shared_lock<std::shared_mutex> lock(m_memoryMutex);
	
	int regionIndex = FindRegionIndex(address);
	if (regionIndex < 0) {
		return 0;  // Default latency for unmapped memory
	}
	
	const MemoryRegion& region = m_regions[regionIndex];
	
	if (region.useVariableLatency && region.latencyCalculator) {
		// Use custom latency calculator
		return region.latencyCalculator(address);
	} else {
		// Use fixed latency
		return region.accessLatency;
	}
}

// Create a method to simulate the access delay
void MemoryManager::SimulateAccessDelay(uint32_t address, bool isWrite, int size) {
	uint32_t latency = GetAccessLatency(address);
	
	if (latency > 0) {
		// In a real system, this would communicate with the CPU to insert wait states
		// For the emulator, we can just log it
		m_logger.Debug("MemoryManager", "Memory access delay: " + std::to_string(latency) + 
					  " cycles for " + (isWrite ? "write to " : "read from ") + 
					  "address 0x" + std::to_string(address) + 
					  " (size: " + std::to_string(size) + " bits)");
		
		// Notify any timing-sensitive components
		NotifyCallbacks(MemoryAccessType::DELAY, address, latency & 0xFF);
	}
}
