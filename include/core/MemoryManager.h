 ...
 */
 ...
 enum class MemoryAccessType {
	...
	DELAY		// Memory delay (address decoding circuitry simulation)
 };
 ...
 struct MemoryRegion {
	...
	// Memory Mirrioring Suport
	bool isMirrored;				 // Whether this region is mirrored
	std::vector<uint32_t> mirrorAddresses; // Addresses where this region is mirrored
	uint32_t mirrorMask;			 // Address mask for mirroring

	// Memory Banking Support
	bool bankable;					// Whether this region supports banking
	std::vector<MemoryBank> banks;	// Different memory banks
	uint32_t currentBankIndex;		// Index of the currently active bank	

	// Address Decoding Simulation
	uint32_t accessLatency;			// Access latency in cycles
	bool useVariableLatency;		   // Whether to use variable latency
	std::function<uint32_t(uint32_t)> latencyCalculator; // Custom latency calculator
 ...
 /**
  * Defines a memory bank in the system
  */
 struct MemoryBank {
	std::string name;
 ...
 private:
	...
	// Dynamic Region Reconfig Methods (TODO: ADD Documentation)
	bool ReconfigureRegion(const std::string& name, uint32_t newSize, MemoryAccess newAccess);
	bool ResizeRegion(const std::string& name, uint32_t newSize);
	bool ChangeRegionAccess(const std::string& name, MemoryAccess newAccess);
	bool RelocateRegion(const std::string& name, uint32_t newStartAddress);

	// Memory Mirroring Support Methods (TODO: ADD Documentation)
	bool CreateMirror(const std::string& sourceName, uint32_t mirrorAddress);
	bool RemoveMirror(const std::string& sourceName, uint32_t mirrorAddress);
	void SetMirrorMask(const std::string& name, uint32_t mask);

	// Memory Banking Support (TODO: ADD Documentation)
	bool AddMemoryBank(const std::string& regionName, uint32_t bankId, uint32_t size);
	bool SwitchBank(const std::string& regionName, uint32_t bankId);
	bool LoadBankData(const std::string& regionName, uint32_t bankId, const std::vector<uint8_t>& data);
	uint32_t GetCurrentBank(const std::string& regionName);

	// Address Decoding Simulation (TODO: ADD Documentation)
	void SetRegionLatency(const std::string& name, uint32_t latencyCycles);
	void SetVariableLatency(const std::string& name, std::function<uint32_t(uint32_t)> calculator);
	uint32_t GetAccessLatency(uint32_t address);
	void SimulateAccessDelay(uint32_t address, bool isWrite, int size);
